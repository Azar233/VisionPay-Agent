...
- 支持通过聊天附件提交 MP4、AVI、MOV、MKV 视频；默认启用 ByteTrack 跨帧跟踪，返回关键帧、按唯一轨迹去重的数量统计和 H.264 标注视频。
- Android IP Webcam MJPEG 实时检测、ByteTrack 轨迹去重和实时计价；结算端支持“累计/瞬时”两种扫描模式。
- 返回类别、置信度、边界框、标注图、推理耗时和商品价格汇总。
- Supervisor 首选由 DeepSeek 驱动的 LLM 路由器，将请求交给 Detection、Dataset、Training、Catalog 或 Knowledge Agent，支持单 Agent、多 Agent 并行和按依赖顺序执行的流水线三种模式，并通过 SSE 流式返回结果。
...
每条管理端消息首选由 DeepSeek 驱动的 LLM 路由器判断归属（temperature 0、20 秒超时，未配置密钥时直接跳过）。路由器输出严格 JSON，将用户意图拆分为一个或多个 Agent 的自包含子任务，并给出三种执行模式之一：

- `single`：单 Agent 处理。
- `parallel`：多个 Agent 相互独立并发执行。
- `pipeline`：多个 Agent 存在依赖，按顺序执行并把前序结果注入下游。

多 Agent 结果由 Supervisor 汇总为统一回复；即使某个 Agent 返回参数表单、页面交接或确认卡片，其余 Agent 的回答仍会照常汇总展示。数据集加样等流程中含样品字段的消息有一条会话延续保护，会直接进入 Dataset Agent。

LLM 路由不可用（未配置密钥、超时或输出非法）时，系统按可审计的降级链依次判断：附件检测与强业务意图规则、未完成工作流延续、`text-embedding-v4` 语义候选、关键词和上一轮 Agent，最终交给 Knowledge Agent 澄清。降级链中的强意图规则仍可覆盖语义候选，例如“向 `mutation-smoke-v2` 添加新的商品”进入 Dataset Agent，“启动训练”进入 Training Agent；覆盖时记录路由日志。
...
默认混合路由首选 LLM 路由器：它为每条消息生成 Agent 归属、执行模式和子任务拆分，结构化会话状态作为路由上下文参与判断。LLM 不可用时的降级优先级为：附件和明确意图规则、未完成工作流、Embedding 语义匹配、关键词和上一轮 Agent，最终无法判断时交给 Knowledge Agent 澄清。`embedding_only` 仅用于纯向量路由测试，不建议作为生产默认值。
...
| 训练与推理 | Ultralytics YOLOv11（含 ByteTrack 跟踪）、PyTorch、OpenCV、Pillow、imageio-ffmpeg |
...
知识库、故障案例、长期记忆和路由降级链中的语义候选使用阿里云百炼 `text-embedding-v4` 与本地 Chroma。生产路由首选 DeepSeek LLM 路由器，Embedding 语义候选只在降级链和 `embedding_only` 评估模式中使用：
...
如需单独评估向量路由，可在 `backend/.env` 临时设置 `AGENT_ROUTING_MODE=embedding_only` 后重启后端。该模式跳过 LLM 路由以及强意图、关键词、附件、未完成工作流和会话上下文保护，最终 Agent 仅由 embedding 相似度决定；Embedding 不可用或低于阈值时安全返回 Knowledge Agent。测试结束必须改回 `AGENT_ROUTING_MODE=hybrid`。
...
视频检测默认启用 ByteTrack 跟踪：按唯一轨迹去重计数、统计峰值同时出现的商品数，并生成 H.264 标注视频（imageio-ffmpeg 编码失败时降级为 MJPG AVI），结果卡内可直接播放。时长超过 `VIDEO_TRACKING_MAX_SECONDS` 的视频会被拒绝；关闭 `VIDEO_TRACKING_ENABLED` 后退回均匀抽帧模式。
...
4. 后端只接受局域网或本机回环的 HTTP 地址（路径限 `/video` 或 `/videofeed`，不允许公网地址和凭据），并代理 MJPEG 流与实时推理结果。

实时检测使用 ByteTrack 轨迹跟踪减少短暂漏检、画面抖动和重复计数：轨迹连续命中或达到高置信度后确认，连续漏检若干帧后过期，每条轨迹只计数一次。结算端提供“累计/瞬时”两种扫描模式：累计模式把本次扫描会话中所有确认过的唯一轨迹计入商品清单，扫过即入账、移出画面不清零，并支持重置扫描；瞬时模式只统计当前画面内的商品。默认瞬时模式。CPU 环境可降低 `CAMERA_IMAGE_SIZE` 或 `CAMERA_TARGET_FPS`。
...
| `VIDEO_TRACKING_ENABLED` | `true` | 视频检测启用 ByteTrack 跟踪与去重计数 |
| `VIDEO_TRACKING_MAX_SECONDS` | `120` | 跟踪模式视频时长上限（秒），超出拒绝 |
| `VIDEO_TRACK_FRAME_STRIDE` | `2` | 跟踪帧步长，每 N 帧推理一次 |
...
| `CAMERA_IMAGE_SIZE` | `640` | 推理输入尺寸 |
| `CAMERA_TARGET_FPS` | `25` | 目标检测帧率 |
...
| `CAMERA_STABILITY_MIN_HITS` | `3` | 轨迹确认最少连续命中数 |
| `CAMERA_STABILITY_MAX_MISSES` | `3` | 轨迹过期允许连续漏检数 |
| `CAMERA_STABILITY_IOU` | `0.25` | 旧帧间稳定器的匹配 IoU，改用 ByteTrack 后保留未参与匹配 |
