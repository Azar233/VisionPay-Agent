<template>
  <section class="result-card">
    <header class="result-header">
      <div>
        <span class="eyebrow">识别结果</span><strong>{{ headerText }}</strong>
      </div>
    ...
    <div v-if="isVideo" class="video-meta">
      ...
      <el-tag effect="plain"
        >{{ result.video_resolution?.width }} × {{ result.video_resolution?.height }}</el-tag
      >
      <el-tag v-if="isTracked" effect="plain"
        >峰值同时 {{ result.peak_simultaneous || 0 }} 件</el-tag
      >
      <span>{{
        isTracked
          ? 'ByteTrack 跨帧去重统计；遮挡或快速移动可能造成少量误差'
          : '统计为各采样帧检测次数之和，不代表跨帧去重商品数'
      }}</span>
    </div>
    ...
    <section v-if="result.price_summary" class="price-summary">
      <header>
        <div>
          <span>商品总价</span><strong>{{ formatMoney(result.price_summary.total_price) }}</strong>
        </div>
        <span
          class="vp-pill"
        ...
        >
          {{
            result.price_summary.pricing_complete
              ? '价格完整'
              : `${result.price_summary.unpriced_objects} 件未定价`
          }}
        </span>
      </header>
    ...
    <div class="result-images">
      ...
      >
        <button class="preview-button" type="button" @click="openPreview(item.annotated_image)">
          <img
            v-if="item.annotated_image"
            :src="item.annotated_image"
            :alt="`${item.filename} 检测标注图`"
          />
        ...
        <div class="image-meta">
          <strong :title="item.filename">{{ itemTitle(item) }}</strong
          ...
          <el-table :data="item.detections" size="small" max-height="190">
            ...
            <el-table-column label="置信度" width="92"
              ><template #default="scope"
                >{{ (scope.row.confidence * 100).toFixed(1) }}%</template
              ></el-table-column
            >
          </el-table>
        </details>
...
const headerText = computed(() => {
  if (!isVideo.value)
    return `${props.result.total_objects} 件商品 / ${props.result.total_images} 张图片`
  if (isTracked.value)
    return `${props.result.total_objects} 件商品（跨帧去重）/ 跟踪 ${props.result.processed_frames} 帧`
  return `${props.result.total_objects} 次采样检测 / ${props.result.processed_frames} 个关键帧`
})
const classes = computed(() =>
  Object.entries(props.result.class_counts || {}).map(([name, count], index) => ({
    name,
    count,
...
function itemMeta(item) {
  if (isTracked.value)
    return `出现 ${Number(item.duration || 0).toFixed(1)}s · 置信度 ${Math.round((item.best_confidence || 0) * 100)}%`
...
.price-items > div > strong {
  color: $text-primary;
}
.price-items .missing-price {
  color: $warning-color;
}
.result-images {
  display: grid;
...
.preview-button span {
  ...
  border-radius: $border-radius-sm;
  color: #fff;
  background: color-mix(in srgb, $bg-color-dark 76%, transparent);
  opacity: 0;
...
.confidence-details summary::after {
  content: '';
  width: 7px;
  height: 7px;
  flex: none;
...
.confidence-details[open] summary::after {
  transform: rotate(225deg) translate(-1px, 1px);
}
.confidence-details summary span {
  flex: 1;
}
.confidence-details summary small {
  color: $text-placeholder;
  font-size: 10px;
  font-weight: 500;
}
.confidence-details :deep(.el-table) {
  width: 100%;
  border-top: 1px solid $border-color;
}
.image-result :deep(.el-table th.el-table__cell) {
  color: $text-secondary;
  font-weight: 600;
  background: $surface-muted;
}
