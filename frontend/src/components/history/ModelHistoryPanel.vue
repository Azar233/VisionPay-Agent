<template>
  <section class="history-panel">
    ...
    <el-table v-loading="loading" :data="items" row-key="id" empty-text="暂无模型历史记录">
      ...
      <el-table-column label="状态" width="118"><template #default="{ row }"><div class="status-stack"><span class="vp-pill" :class="`vp-pill--${modelStatusType(row.status)}`">{{ modelStatusText(row.status) }}</span><span v-if="row.is_default">默认</span></div></template></el-table-column>
...
function modelStatusType(status) { return ({ active: 'success', archived: 'info', deleted: 'danger' })[status] || 'info' }
function modelStatusText(status) { return ({ active: '活动', archived: '已归档', deleted: '已删除' })[status] || status || '未知' }
