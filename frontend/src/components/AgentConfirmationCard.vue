<template>
  <section class="confirmation-card">
    ...
    <div v-if="operation.impact?.warnings?.length" class="warnings">
      ...
      <ul>
        <li v-for="warning in operation.impact.warnings" :key="warning">{{ warning }}</li>
      </ul>
    </div>
...
const waitForProgressPoll = (duration = 120) =>
...
function compactProgressHistory(history, maxItems = 5) {
  ...
  return Array.from(
    { length: maxItems },
    (_, index) => history[Math.round((index * (history.length - 1)) / (maxItems - 1))],
  )
}

async function replayProgressHistory(petTask, taskProgress, lastDisplayedProgress) {
  const unseen = (taskProgress?.history || []).filter(
    (item) =>
...
const expiresText = computed(() => {
  ...
  return Number.isNaN(date.getTime())
    ? '令牌短时有效'
    : `有效至 ${date.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}`
})
  ...
  return (
    {
      ...
      old_currency: '原币种',
      new_currency: '新币种',
      old_model_version: '原模型',
      new_model_version: '新模型',
      affected_images: '受影响图片',
      annotations_deleted: '删除标注',
      mixed_scene_images_retained: '保留多商品场景',
      classes_reindexed: '重排类别',
      epochs: '训练轮数',
      batch_size: '批次大小',
      img_size: '图片尺寸',
      device: '训练设备',
