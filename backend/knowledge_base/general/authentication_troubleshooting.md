...
401 通常表示 JWT 缺失、无效或过期。前端会合并同一时刻的并发 401，只显示一次过期提示、清理一次登录状态并跳转 `/login`。重新登录后再发起请求，不要反复提交旧确认令牌。Token 有效期由 `ACCESS_TOKEN_EXPIRE_MINUTES` 控制，当前默认值为 10080 分钟（7 天）。
